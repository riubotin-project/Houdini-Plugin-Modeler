    ...
    with hou.RedrawBlock() as rb:
        with hou.undos.group("Modeler: Transform Operation"):
            ...
            if mode.scene_viewer.curViewport().type() == hou.geometryViewportType.UV:
                ...
                if sop is not None:
                    if op == 0:
                        ui.run_internal_command("h.pane.gview.state.select.move")
                    elif op == 1:
                        ui.run_internal_command("h.pane.gview.state.select.rotate")
                    else:
                        ui.run_internal_command("h.pane.gview.state.select.scale")
                return True
...
def LocalTranslate(mode, op=0):
    ...
    if sop is not None:
        ...
        with hou.undos.group("Modeler: Local TRS Operation"):
            with hou.RedrawBlock() as rb:
                sop = geo_utils.get_edit_sop(mode.scene_viewer, sop, parms={"group": items, "grouptype": grouptype, "xformspace": 2, "localframe": 0})
                geo_utils.setup_edit_sop_for_symmetry(sop)
                sop.setHighlightFlag(True)
...
_grab_scene_viewer = None
_grab_objects = None
_grab_objects_translatemethod = None
_grab_start_time = None

def _grab(mode, mods, state_name):
    global _grab_scene_viewer, _grab_objects, _grab_objects_translatemethod, _grab_start_time

    state = mode.state()
    _grab_scene_viewer = mode.scene_viewer
    ...
    # SOP
    elif _grab_scene_viewer.pwd().childTypeCategory() == geo_utils._sop_category:
        sop, sel, pwd = geo_utils.sop_selection(_grab_scene_viewer)
        if sop is not None:
            geo = sop.geometry()
            ...
            with hou.undos.group("Modeler: Grab Geometry"):
                with hou.RedrawBlock() as rb:
                    node = geo_utils.get_edit_sop(_grab_scene_viewer, sop)
                    ...
                    _grab_scene_viewer.setCurrentState(state_name)

                    _grab_start_time = mode.key_tool_time

                    LeftButtonDrag(mode, mods, release_callback=_finish_grab_callback)
        ...
        if _grab_objects:
            with hou.undos.group("Modeler: Grab Objects"):
                if mods == ui.qt.ShiftModifier:
                    h = ui.get_hvd(_grab_scene_viewer, True)[0]
                    if h[0]:
                    ...
                    v = ui.get_hvd(_grab_scene_viewer, True)[1]
                    if v[0]:
                        for node in _grab_objects:
                            node.parm("ty").disable(True)
                            node.parm("tz").disable(True)
                    ...
                    d = ui.get_hvd(_grab_scene_viewer, True)[2]
                    if d[0]:
                        for node in _grab_objects:
                            node.parm("tx").disable(True)
                    elif d[1]:
                ...
                with hou.RedrawBlock() as rb:
                    ...
                    _grab_scene_viewer.curViewport().settings().enableGuide(hou.viewportGuide.NodeHandles, False)
                    _grab_scene_viewer.enterTranslateToolState()

                    MiddleButtonDrag(mode, release_callback=_finish_grab_objects_callback)
...
def _finish_grab_callback():
    if ( ui.time.time() - _grab_start_time ) > 0.2:
        _grab_scene_viewer.setCurrentState("select")

def _finish_grab_objects_callback():
    ui.executeDeferred(__finish_grab_objects_callback)


def __finish_grab_objects_callback():
    for node in _grab_objects:
        node.parmTuple("t").disable(False)
    
    _grab_scene_viewer.curViewport().settings().enableGuide(hou.viewportGuide.NodeHandles, True)

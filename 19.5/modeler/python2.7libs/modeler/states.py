...
class StateTemplate(object):
    ...
    def onMouseEvent(self, kwargs):
        ...
        if reason == hou.uiEventReason.Active:
            ...
            if self._drag_type == 3 and self.allow_snapping:
                ...
                # NOT SNAPPED
                    ...
                    delta_vector = geo_utils.intersect_plane(self._start_drag_pos,  self._current_viewport_plane_dir, *ui_event.ray()) - self._start_drag_pos
            ...
            # FLOAT DISTANCE
                ...
                delta_vector = geo_utils.intersect_plane(self._start_drag_pos,  hou.Vector3(0.0, 0.0, 1.0) * self._viewport_xform_inverted_transposed * self._xform_transposed, *ui_event.ray()) - self._start_drag_pos
        ...
        # POINTER PRESS
            ...
            if hit_info is None:
                self._start_drag_pos = geo_utils.intersect_plane(self.geo.boundingBox().center(), hou.Vector3(0.0, 0.0, 1.0) * self._viewport_xform_inverted_transposed * self._xform_transposed, self._start_drag_ray_origin, self._start_drag_ray_direction)
            else:
...
def Push(mode, from_menu_action=False):
    ...
    if sop is not None:
        if sop.type().name() == "edit" and sop.cookCount() < 7:
            sop.destroy()
    ...
    def onMouseEvent(self, kwargs):
        ...
        if reason == hou.uiEventReason.Active:
            ...
            if self.transform_mode == 1:
                o, d = event.ray()
                intersection = geo_utils.intersect_plane(self._start_intersection, self._view_normal_local, o, d)

                if self._is_sym:
                    intersection = geo_utils.intersect_plane(self._start_intersection, self._sym_dir, intersection, self._sym_dir)
            ...
            if x != self.last_mouse_move_x or y != self._last_mouse_move_y:   
                ...
                if intersected > -1:
                    ...
                    if self.sop.evalParm("doreflect"):
                        ...
                        i = geo_utils.intersect_plane(sym_origin, self._sym_dir, self._hit_point_pos, -self._sym_dir)
